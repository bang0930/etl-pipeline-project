...
- 동일한 작업을 다시 실행해도 중복 데이터가 발생하지 않는다.
...
한 행은 한 번의 API 페이지 응답을 의미하며, 다음 정보를 저장할 예정이다.
...
날짜는 `DATE`, 정수형 가격과 거래 데이터는 `BIGINT`, 등락률은 `NUMERIC`으로 변환할 예정입니다. Raw 응답의 식별자도 함께 저장하여 가공된 데이터의 출처를 추적한다.
...
ETL 구현을 진행하면서 데이터베이스 DDL, 단계별 ETL 모듈, Airflow DAG와 테스트 디렉터리를 추가할 예정이다.
...
실제 `.env` 파일은 Git으로 추적하지 않습니다. 저장소에는 변수명만 제공하는 `.env.example`만 포함한다.

## 향후 계획

다음 순서로 주식 시세 파이프라인을 구현한다.

1. `raw`, `staging`, `mart` 스키마와 Raw 테이블 DDL 작성
2. 지정한 기준일자와 페이지의 주식 시세를 수집하는 Extract 구현
3. 전체 페이지 수집과 네트워크 오류 재시도 구현
4. API 응답과 수집 메타데이터를 Raw 계층에 적재
